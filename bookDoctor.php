...
<head>
  ...
  <!-- <link rel="stylesheet" href="style/bookDoctor1.css"> -->
  <style>
  </style>
</head>
<body>
...
// "All" link
echo '<a href="bookDoctor.php?partial=1"' . ($current === '' ? ' class="active filter-link"' : ' class="filter-link"') . '>All</a>';

foreach ($departments as $dept) {
    $class = ($dept === $current) ? ' class="active filter-link"' : ' class="filter-link"';
    echo ' <a href="bookDoctor.php?specialist=' . urlencode($dept) . '&partial=1"' . $class . '>' . htmlspecialchars($dept) . '</a>';
}
?>
    <!-- Search Form -->
<form method="GET" class="search-form" action="bookDoctor.php">
...
<?php else: ?>
  <button onclick="alert('You need to login first!'); window.location.href='login.php';" class="profile-button">
    Doctor's Profile
  </button>
<?php endif; ?>
